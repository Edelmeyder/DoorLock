...
static int8_t index = 0;            // posici?n en tempValue donde se est? escribiendo

static int8_t digits[] = {1, 2, 3, 0, 4, 5, 6, 0, 7, 8, 9, 0, 0, 0}; 

/*
	Guarda la hora con lo que se carg? hasta el momento
*/
...
/*
	Guarda los minutos con lo que se carg? hasta el momento
*/
...
/*
	Guarda los segundos con lo que se carg? hasta el momento
*/
...
void Clockhms_Tick(void)
{
   ...
   LCD_sendChar((hours/10) + '0');
   LCD_sendChar((hours%10) + '0');
   LCD_sendChar( ':');
   LCD_sendChar((minutes/10) + '0');
   LCD_sendChar((minutes%10) + '0');
   LCD_sendChar(':');
   LCD_sendChar((seconds/10) + '0');
   LCD_sendChar((seconds%10) + '0');
}
...
/*
  Almacena temporalmente el d?gito que se env?a.
